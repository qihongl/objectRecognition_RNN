...
function [pattern, numCategory] = patternGen(filename, thres1)
    ...
    if exist(filename, 'file') == 0
        disp('Make sure the PROTO file is in the working directory')
    else
        temp = xlsread(filename);
        % the 1st line contains the metadata
        ...
        numInstances = size(temp,1) - 1;
        prototype = temp(2:end,:);
...
%     blankMatrix = zeros(size(prototype,1),size(prototype,2));
